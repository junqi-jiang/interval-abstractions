...
def get_flattened_weight_and_bias(model):
    ...
    if model._h is None:
        ...
        return np.append(w_all.flatten(), b_all.flatten())
...
def cross_validation(d, num_h_neurons=20, epochs=50, data_name="adult", linear=False):
    torch.manual_seed(5)
    np.random.seed(1)
